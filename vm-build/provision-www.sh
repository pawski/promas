...
cp /vagrant/config/www/promas.local /etc/nginx/sites-available/
ln -s /etc/nginx/sites-available/promas.local /etc/nginx/sites-enabled/promas.local
...
echo "=> Configuring /promas directory"
ln -s /var/www/ /promas

echo 'export PATH=/promas/bin/:$PATH' >> /home/vagrant/.profile
